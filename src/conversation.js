...
app.get("/api/conversation", async (req, res) => {
  ...
  if (!conversationId || !storeId) {
    return res
      .status(400)
      .json({
        error: "Missing required parameters: conversationId and storeId",
      });
  }
  ...
  try {
    ...
    // Adjusting the mapping to access productUrl correctly
        ...
        if (message.type === "card" && Array.isArray(message.cards)) {
          return message.cards.map((card) => ({
            ...
            content: {
              ...
              productUrl: card.content.productUrl || "", // Correct access path
